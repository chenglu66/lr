...
def stocGradAscent2(x, y, theta, alpha): 
    ...
    for j in range(numIter):
        ...
        for i in range(m):
            ...
            h = sigmode(dot(x[randIndex,:],theta))
            error = y[randIndex] - h
            theta -= (alpha * error * x[randIndex,:]).T
            del(dataIndex[randIndex])
    return theta
if __name__=='__main__':
   x,y=loaddata('E:\下载\machine-learning-ex-master\machine-learning-ex2\ex2\ex2data1.txt')
   m,n = shape(x)
   t=stocGradAscent2(x, y, theta, alpha=0)
   #t=stocGradAscent1def(x, y, theta, alpha=0.001)
   #t=gradAscent(x, y)
   #print(weight)
   t=t.tolist()
   #print(t)
   ...
   temp1=mat(temp1)
   x1=arange(20,110,10)
   y=(float(t[1][0])+float(t[1][0])*x1)/-float(t[2][0])
   fig=plt.figure()
